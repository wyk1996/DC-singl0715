...
//		SetBcuRemoteSignalState(gun,(INT8U)REMOTE_SIGNALE_1, 7, 0);   //继电器粘连拔枪清除   20211118
	...
	{
		SetBcuRemoteSignalState(gun,(INT8U)REMOTE_SIGNALE_3, 3, 0); //直流母线输出不过流告警0
	}
...
#endif
	{
		SetBcuRemoteSignalState(gun,(INT8U)REMOTE_SIGNALE_3, 4, 0);   //直流母线输出不过流告警0
	}
...
//		 ((BCURemoteSignal[gun].state3.byte&0xFC)!=0)||((BCURemoteSignal[gun].state4.byte&0xE4)!=0) || BSP_MPLSState(BSP_DI_DOOR))
		...
		if(((BCURemoteSignal[gun].state1.byte&0xB0)!=0)||((BCURemoteSignal[gun].state2.byte&0x3C)!=0)||\
			((BCURemoteSignal[gun].state3.byte&0xEC)!=0)||((BCURemoteSignal[gun].state4.byte&0xE4)!=0) || BSP_MPLSState(BSP_DI_DOOR))  //如果是模块欠压时，上面是一个是FC和一个EC，控制模块故障的标志位灯不亮了
		{
			if(state[gun])
			{
				state[gun] = 0;
...
void TaskControlGunA(void *pdata)
{
	...
	OSTimeDly(SYS_DELAY_1s);
	APP_ControlInit();  //刚上电就把状态清空一下（例：充电未开始、是否需要自检等很多）
	OSTimeDly(SYS_DELAY_2s);
//	BSP_OpenLOCK(GUN_A);
//	BSP_OpenLOCK(GUN_B);
//

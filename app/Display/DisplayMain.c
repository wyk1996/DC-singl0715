...
static INT8U Munu52_MacMaxVolDispose(INT16U addr,INT8U *pvalue,INT8U len)
{
	...
	PresetValue.Therold_Value.OverOutVolTherold = PresetValue.VolMaxOutput+150;   //设备最高电压阀值+15V
...
static INT8U Munu52_MaxOutCurDispose(INT16U addr,INT8U *pvalue,INT8U len)
{
	...
	PresetValue.Therold_Value.OverOutCurTherold = PresetValue.CurMaxOutput+100;   //设备最大电流阀值
...
* Contributor	: 2018年7月18日
***********************************************************************************************/
INT8U Munu52_SOCDispose(INT16U addr,INT8U *pvalue,INT8U len)
{
	INT8U SOC = 0;
	_FLASH_OPERATION  FlashOper;
	if((pvalue == NULL) || (len != 2))
...
//网关自定义
static INT8U Munu85_WG1Sert(INT16U addr,INT8U *pvalue,INT8U len)
{
	INT8U temp;
	_FLASH_OPERATION  FlashOper;
	if((pvalue == NULL) || (len != 2) )
...
static INT8U Munu85_WG2Sert(INT16U addr,INT8U *pvalue,INT8U len)
{
	INT8U temp;
	_FLASH_OPERATION  FlashOper;
	if((pvalue == NULL) || (len != 2) )
...
static INT8U Munu85_WG3Sert(INT16U addr,INT8U *pvalue,INT8U len)
{
	INT8U temp;
	_FLASH_OPERATION  FlashOper;
	if((pvalue == NULL) || (len != 2) )
...
static INT8U Munu85_WG4Sert(INT16U addr,INT8U *pvalue,INT8U len)
{
	INT8U temp;
	_FLASH_OPERATION  FlashOper;
	if((pvalue == NULL) || (len != 2) )
...
//子网掩码设置
static INT8U Munu85_ZW1Sert(INT16U addr,INT8U *pvalue,INT8U len)
{
	INT8U temp;
	_FLASH_OPERATION  FlashOper;
	if((pvalue == NULL) || (len != 2) )
...
static INT8U Munu85_ZW2Sert(INT16U addr,INT8U *pvalue,INT8U len)
{
	INT8U temp;
	_FLASH_OPERATION  FlashOper;
	if((pvalue == NULL) || (len != 2) )
...
static INT8U Munu85_ZW3Sert(INT16U addr,INT8U *pvalue,INT8U len)
{
	INT8U temp;
	_FLASH_OPERATION  FlashOper;
	if((pvalue == NULL) || (len != 2) )
...
static INT8U Munu85_ZW4Sert(INT16U addr,INT8U *pvalue,INT8U len)
{
	INT8U temp;
	_FLASH_OPERATION  FlashOper;
	if((pvalue == NULL) || (len != 2) )
...
//固定IP地址
static INT8U Munu85_IP1Sert(INT16U addr,INT8U *pvalue,INT8U len)
{
	INT8U temp;
	_FLASH_OPERATION  FlashOper;
	if((pvalue == NULL) || (len != 2) )
...
static INT8U Munu85_IP2Sert(INT16U addr,INT8U *pvalue,INT8U len)
{
	INT8U temp;
	_FLASH_OPERATION  FlashOper;
	if((pvalue == NULL) || (len != 2) )
...
static INT8U Munu85_IP3Sert(INT16U addr,INT8U *pvalue,INT8U len)
{
	INT8U temp;
	_FLASH_OPERATION  FlashOper;
	if((pvalue == NULL) || (len != 2) )
...
static INT8U Munu85_IP4Sert(INT16U addr,INT8U *pvalue,INT8U len)
{
	INT8U temp;
	_FLASH_OPERATION  FlashOper;
	if((pvalue == NULL) || (len != 2) )
...
const _DISP_ADDR_FRAME Disp_RecvFrameTable[] =
{
	...
	{DIS_ADD(85,0x05)				,		Munu85_WG1Sert		},			//85页面设置网关
	{DIS_ADD(85,0x06)				,		Munu85_WG2Sert		},			//ip设置
	{DIS_ADD(85,0x07)				,		Munu85_WG3Sert		},			//ip设置
	{DIS_ADD(85,0x08)				,		Munu85_WG4Sert		},			//ip设置

	{DIS_ADD(85,0x09)				,		Munu85_ZW1Sert		},			//85页面设置子网掩码
	{DIS_ADD(85,0x0A)				,		Munu85_ZW2Sert		},			//
	{DIS_ADD(85,0x0B)				,		Munu85_ZW3Sert		},			//
	{DIS_ADD(85,0x0C)				,		Munu85_ZW4Sert		},			//

	{DIS_ADD(85,0x0D)				,		Munu85_IP1Sert		},			//85页面IP地址设置
	{DIS_ADD(85,0x0E)				,		Munu85_IP2Sert		},			//
	{DIS_ADD(85,0x0F)				,		Munu85_IP3Sert		},			//
	{DIS_ADD(85,0x10)				,		Munu85_IP4Sert		},			//
...
static INT8U APP_RecvCardAnalyze(_BSP_MESSAGE *pMsg)
{
	...
	switch(pMsg->DivNum)
	{
		...
		case NETCARDCMD_STARTA:		  //A枪网络开始刷卡
			...
			if((SYSSet.NetYXSelct == XY_ZSH)||(SYSSet.NetYXSelct == XY_XDT)||(SYSSet.NetYXSelct == XY_TTC)||(SYSSet.NetYXSelct == XY_HY)||(SYSSet.NetYXSelct == XY_YL2)||(SYSSet.NetYXSelct == XY_YD)||(SYSSet.NetYXSelct == XY_GZZJ)||(SYSSet.NetYXSelct == XY_TT)||(SYSSet.NetYXSelct == XY_JG)||(SYSSet.NetYXSelct == XY_YKC)||(SYSSet.NetYXSelct == XY_YL3)||(SYSSet.NetYXSelct == XY_YL4)||(SYSSet.NetYXSelct == XY_XJ)||(SYSSet.NetYXSelct == XY_YL1))
			{
				if(DispControl.NetGunState[GUN_A] != GUN_IDLE)
				{
					DisplayCommonMenu(HYMenu22.Menu_PrePage,NULL);
		...
		case NETCARDCMD_STOPA:		 //A枪网络停止充电
			if((SYSSet.NetYXSelct == XY_ZSH)||(SYSSet.NetYXSelct == XY_XDT)||(SYSSet.NetYXSelct == XY_TTC)||(SYSSet.NetYXSelct == XY_AP)||(SYSSet.NetYXSelct == XY_HY)||(SYSSet.NetYXSelct == XY_TT)||(SYSSet.NetYXSelct == XY_JG)||(SYSSet.NetYXSelct == XY_YL2)||(SYSSet.NetYXSelct == XY_YD)||(SYSSet.NetYXSelct == XY_GZZJ)||(SYSSet.NetYXSelct == XY_YKC)||(SYSSet.NetYXSelct == XY_YL3)||(SYSSet.NetYXSelct == XY_YL4)||(SYSSet.NetYXSelct == XY_XJ)||(SYSSet.NetYXSelct == XY_YL1))
			{
		...
		case NETCARDCMD_STOPB:		 //B枪网络停止充电
			if((SYSSet.NetYXSelct == XY_ZSH)||(SYSSet.NetYXSelct == XY_XDT)||(SYSSet.NetYXSelct == XY_TTC)||(SYSSet.NetYXSelct == XY_AP)||(SYSSet.NetYXSelct == XY_HY)||(SYSSet.NetYXSelct == XY_JG)||(SYSSet.NetYXSelct == XY_YL2)||(SYSSet.NetYXSelct == XY_YD)||(SYSSet.NetYXSelct == XY_GZZJ)||(SYSSet.NetYXSelct == XY_YKC)||(SYSSet.NetYXSelct == XY_TT)||(SYSSet.NetYXSelct == XY_YL3)||(SYSSet.NetYXSelct == XY_YL4)||(SYSSet.NetYXSelct == XY_XJ)||(SYSSet.NetYXSelct == XY_YL1))
			{
				if((FRAMERTU)(*pMsg->pData) == _SuccessRtu)
				{
	...
	if(SYSSet.NetState == DISP_NET)
	{
		switch(pMsg->DivNum)
		{
			case APP_CHARGE_FAIL:            //启动失败
				...
				if((SYSSet.NetYXSelct == XY_HY)||(SYSSet.NetYXSelct == XY_YL2)||(SYSSet.NetYXSelct == XY_YD)||(SYSSet.NetYXSelct == XY_GZZJ)||(SYSSet.NetYXSelct == XY_JG))
				{
			...
			case APP_CHARGE_SUCCESS:         //启动成功
				...
				{
					if((SYSSet.NetYXSelct == XY_AP)||(SYSSet.NetYXSelct == XY_HY)||(SYSSet.NetYXSelct == XY_JG)||(SYSSet.NetYXSelct == XY_YL2)||(SYSSet.NetYXSelct == XY_YD)||(SYSSet.NetYXSelct == XY_GZZJ)||(SYSSet.NetYXSelct == XY_66))
					{
...
//	SHOW_MODULE_YL_15KW,
	...
	{
		Dis_Show(DIS_ADD(HYMenu57.FrameID,0),SHOW_NET_DJ); //单机
	}
	else
	{
		if(SYSSet.Net4GorEternet == NET_4G)
		{
			Dis_Show(DIS_ADD(HYMenu57.FrameID,0),SHOW_NET_WL);  //4G
		}
		else if(SYSSet.Net4GorEternet == NET_Ethernet)
		{
			Dis_Show(DIS_ADD(HYMenu57.FrameID,0),SHOW_NET_Eternet); //显示以太网
		}
...
void Display_ShowNet(void)
{
	...
	else if(SYSSet.NetYXSelct == 11)
	{
		Dis_Show(DIS_ADD(HYMenu10.FrameID,0),SHOW_XY_ZSH);
	}
	else if(SYSSet.NetYXSelct == 12)
	{
		Dis_Show(DIS_ADD(HYMenu10.FrameID,0),SHOW_XY_YD);  
	}
	else if(SYSSet.NetYXSelct == 13)
	{
		Dis_Show(DIS_ADD(HYMenu10.FrameID,0),SHOW_XY_XDT);  
	}
	else if(SYSSet.NetYXSelct == 14)
	{
		Dis_Show(DIS_ADD(HYMenu10.FrameID,0),SHOW_XY_GZZJ);  
	}
	else if(SYSSet.NetYXSelct == 15)
	{
		Dis_Show(DIS_ADD(HYMenu10.FrameID,0),SHOW_XY_TTC);  
	}
	...
	INT16U bufINterIP[16]; //以太网IP设置
	for(i = 0; i < 4; i++)
	{
		bufINterIP[i] = (SYSSet.gateway[i] & 0x00ff) << 8 | 0;
	}

	for(i = 0; i < 4; i++)
	{
		bufINterIP[4+i] = (SYSSet.DHCPZWIP[i] & 0x00ff) << 8 | 0;
	}

	for(i = 0; i < 4; i++)
	{
		bufINterIP[8+i] = (SYSSet.DHCPip[i] & 0x00ff) << 8 | 0;
	}
	PrintStr(DIS_ADD(HYMenu85.FrameID,0x05),(INT8U *)bufINterIP ,sizeof(bufINterIP)); //从85页面开始05键开始
...
static void Display_ParaInit(void)
{
	...
	if((SYSSet.SOCthreshold < 20)||(SYSSet.SOCthreshold >100))  //JG的要求20-100之间
	{
	...
	if(((SYSSet.gateway[0] == 0xFF) && (SYSSet.gateway[1] == 0xFF) && (SYSSet.gateway[2] == 0xFF)) || \
	        ((SYSSet.DHCPip[0] == 0XFF)&&(SYSSet.DHCPip[1] == 0XFF)&&(SYSSet.DHCPip[2] == 0XFF)))
	{
		SYSSet.gateway[0] = 192; //默认网关
		SYSSet.gateway[1] = 168;
		SYSSet.gateway[2] = 1;
		SYSSet.gateway[3] = 1;

		SYSSet.DHCPip[0] = 192;  //默认Ip地址，子网掩码暂时不写
		SYSSet.DHCPip[1] = 168;
		SYSSet.DHCPip[2] = 1;
		SYSSet.DHCPip[3] = 10;
	}

	if((SYSSet.DHCPZWIP[0] == 0xFF) && (SYSSet.DHCPZWIP[1] == 0xFF) && (SYSSet.DHCPZWIP[2] == 0xFF) && (SYSSet.DHCPZWIP[3] == 0xFF))
	{
		SYSSet.DHCPZWIP[0] = 255;   //默认网关
		SYSSet.DHCPZWIP[1] = 255;
		SYSSet.DHCPZWIP[2] = 255;
		SYSSet.DHCPZWIP[3] = 0;
	}
  ...
  //标志位清空  //写入flash  防止还源
	...
	if((SYSSet.NetYXSelct == XY_ZSH)||(SYSSet.NetYXSelct == XY_XDT)||(SYSSet.NetYXSelct == XY_TTC)||(SYSSet.NetYXSelct == XY_YKC)||(SYSSet.NetYXSelct == XY_TT)||(SYSSet.NetYXSelct == XY_YL3)||(SYSSet.NetYXSelct == XY_YL4)||(SYSSet.NetYXSelct == XY_XJ)||(SYSSet.NetYXSelct == XY_YL1))
	{
		//云快充  当充电中修改费率，A枪的费率可能和B枪的费率不一样
	...
	FlashOper.Len = sizeof(PresetValue);
	FlashOper.ptr = (INT8U*)&PresetValue;
	FlashOper.RWChoose = FLASH_ORDER_WRITE;  //写flash
	APP_FlashOperation(&FlashOper);
...
//===以太网的wifi标志时是全亮
static INT8U DispShow_NET_EthernetState(void)
{
	static INT8U state = 0,laststate = 0xff;   //变化了才执行
...
static INT8U DispShow_State(INT32U time)
{
	...
	for(i = 0; i < num; i++)
	{
		...
		if((APP_GetErrState((_GUN_NUM)i) == 0x04) || (GetMeterStatus((_GUN_NUM)i) == _COM_FAILED)) 				//有故障
		{
			...
			{
				show_err[(_GUN_NUM)i] = SHOW_SYS_EMERGENCY;  //急停故障
			}
			else if (BSP_MPLSState(BSP_DI_DOOR)) 
			{
				show_err[(_GUN_NUM)i] = SHOW_SYS_MJ;  //门禁故障
			}
#if ENBLE_JG
		...
		if(SYSSet.Net4GorEternet == NET_Ethernet)
		{
			DispShow_NET_EthernetState();  //单独的以太网图标
			DispShow_NetState();     //后台标示已经收到注册成功（如果没有）
		}
		else if(SYSSet.Net4GorEternet == NET_4G)   //单独4g时
		{
			DispShow_CSQState();   //wifi图标
			DispShow_NetState();   //后台标示注册成功
		}
		else
		{
			__NOP();
	...
	if(((SYSSet.NetYXSelct == XY_ZSH)||(SYSSet.NetYXSelct == XY_XDT)||(SYSSet.NetYXSelct == XY_TTC)||(SYSSet.NetYXSelct == XY_YKC)||(SYSSet.NetYXSelct == XY_TT)||(SYSSet.NetYXSelct == XY_YL3)||(SYSSet.NetYXSelct == XY_YL4)||(SYSSet.NetYXSelct == XY_XJ)||(SYSSet.NetYXSelct == XY_YL1))&&(SYSSet.NetState == DISP_NET))
	{
		for(i = 0; i < GUN_MAX; i++)
		{
...
static INT8U Period_WriterFmBill(INT32U time)
{
	...
	{
		if(++count[GUN_A] >= ((SYS_DELAY_5M)/time) )   //这是大概5分钟存储一次（大概就是5分钟存储1次）
			//if(++count[GUN_A] >= ((SYS_DELAY_5s)/time) )  //这个是联调测试可以用（5s存储1次）
		{
	...
	{
		if(++count[GUN_B] >= ((SYS_DELAY_5M)/time) )   //这是大概5分钟存储一次
			//	if(++count[GUN_B] >= ((SYS_DELAY_5s)/time) )   //这个是联调测试可以用（5s存储1次）
		{
...
INT8U DisplayGunQRCode(_GUN_NUM gun)
{
	...
	char * acnet = "https://www.chuangyuechongdian.com/cpile/";  //安充
	char * YDnet = "https://charging.yidianone.com/charging/pile/";  //易电
	char * GZZJnet = "https://zj.fengjingit.com/cpile/"; //贵州中交
	...
	char * ykzsh = "https://ne.gdsz.sinopec.com/h5/MPAGE/index.html?pNum=";   //中石化
	...
	char * XDTnet = "https://napi.bangdao-tech.com?No=";	 //新电途
	char * TTC_net = "https://ttcomp.evchong.com/LUOSHU-MP-PAGE/index.html?pNum=";	 //淘淘充
	...
	if(SYSSet.NetYXSelct == XY_XDT)  //新电途二维码
	{
		len = strlen(XDTnet);
		if(len > 80)
		{
	...
	if(SYSSet.NetYXSelct == XY_TTC)  //淘淘充二维码
	{
		len = strlen(TTC_net);
		if(len > 80)
		{
...
static INT8U Dispose_ACSW(void)
{
	...
	//急停---交流接触器会立马断开
	if((BCURemoteSignal[GUN_A].state1.State.Stop_Pluse == 0x01) || (BCURemoteSignal[GUN_B].state1.State.Stop_Pluse == 0x01))
	{
	   BSP_IOClose(IO_RLY_K3);
	}
...
INT32U  TTlasttime = 0;   //定义一个铁塔84页面全局变量
/*****************************************************************************
...
ST_Menu * Disp_NetPageDispos(void)
{
	...
	//如果界面不等于空闲时，时间复制-----必须是铁塔的协议下
	if(USER_GUN == USER_DOUBLE_GUN)  //双枪判断条件
	{
		if((gun_state[GUN_A] != 0) || (gun_state[GUN_B] != 0)|| (show_err[GUN_A] != SHOW_SYS_GUN) || (show_err[GUN_B] != SHOW_SYS_GUN))
		{
			TTlasttime = OSTimeGet();
		}
	}
	else  //单枪判断条件
	{
		if((gun_state[GUN_A] != 0) || (show_err[GUN_A] != SHOW_SYS_GUN))
		{
			TTlasttime = OSTimeGet();
		}
	}
	...
	{
		if(SYSSet.NetYXSelct == XY_TT)  //单独铁塔的才会有待机初始界面，其它的一切都没有待机界面
		{
			if((OSTimeGet() >= TTlasttime) ? ((OSTimeGet() - TTlasttime) >= SYS_DELAY_30s):((OSTimeGet() + (0xFFFFFFFF - TTlasttime)) >= SYS_DELAY_30s))
			{
				return &HYMenu84;  //铁塔界面---双枪都空闲时，跳转到待机界面
			}
			else
			{
				return &HYMenu73;
...
static void NetCardDispose(INT32U time)
{
	...
	if((SYSSet.NetYXSelct != XY_AP) && (SYSSet.NetYXSelct != XY_HY)&& (SYSSet.NetYXSelct != XY_JG)&&(SYSSet.NetYXSelct != XY_YL2)&&(SYSSet.NetYXSelct != XY_YD)&&(SYSSet.NetYXSelct != XY_GZZJ) \
	        && (SYSSet.NetYXSelct != XY_ZSH)&& (SYSSet.NetYXSelct != XY_XDT)&& (SYSSet.NetYXSelct != XY_TTC)&& (SYSSet.NetYXSelct != XY_YKC)&& (SYSSet.NetYXSelct != XY_TT)&& (SYSSet.NetYXSelct != XY_YL3)&& (SYSSet.NetYXSelct != XY_YL4) && ((SYSSet.NetYXSelct != XY_XJ)) \
...
static void DispPeriod(void)
{
	...
	{
		lastMenu =CurMenu;
		//版本号：DC240530-1-1   第一个1是单枪,  第二个是看门狗是否开启(0关闭，1开启)
		snprintf((char *)buf, sizeof(buf), "DC%02d%02d%02d-%d-%d",SYS_APP_VER[0],SYS_APP_VER[1],SYS_APP_VER[2],USER_GUN+1,USER_dog);  
		PrintStr(0x0100,(INT8U *)buf,strlen(buf)); //程序版本
		PrintStr(0x0200,(INT8U *)SYSSet.SysSetNum.DivNum,sizeof(SYSSet.SysSetNum.DivNum)); //桩号
	}
	...
	if(SYSSet.NetState == DISP_NET)
	{
		if((CurMenu == &HYMenu73) || (CurMenu == &HYMenu74) || (CurMenu == &HYMenu75) || (CurMenu == &HYMenu76) \
		        || (CurMenu == &HYMenu77) || (CurMenu == &HYMenu78) || (CurMenu == &HYMenu79) ||  (CurMenu == &HYMenu80) || (CurMenu == &HYMenu81)|| (CurMenu == &HYMenu84))
		{
			ppage = Disp_NetPageDispos();

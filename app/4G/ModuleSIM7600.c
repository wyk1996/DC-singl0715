...
//设置4g模块是否存在？？？
INT8U APP_SetSIM7600Status(_4G_STATE state)
{
	SIM7600CmdState.SIM_ExistState = state;
	return TRUE;
}
...
INT8U APP_SetModuleConnectState(INT8U num,_4G_STATE state)
{
	if(num > 10)
	{
		return FALSE;
	}
	SIM7600CmdState.SIM_ConectState[num] = state;
	return TRUE;
}
...
//	static INT8U count = 1;
	...
	SIM7600CmdState.SIM_ExistState = STATE_OK;		//4G模块存在
...
{
	if(SYSSet.NetState == DISP_NET)  //必须是网络模式下----下面又分为4g 或是 以太网
	{
		INT8U i = 0,err = 0;
...
//	}
		...
		{
			int16_t size = send(num,pdata,len);  //以太网 发送数据
			if(size != len)
			{
				printf("cuowu-size = %d\n",size);
				return 1;
			...
			{
				printf("INtersendlen:%d\n",size);
			}
		}

		OSMutexPost(SendMutex); //释放锁
		if((SYSSet.NetYXSelct == XY_ZSH)||(SYSSet.NetYXSelct == XY_XDT)||(SYSSet.NetYXSelct == XY_TTC)||(SYSSet.NetYXSelct == XY_YKC))
		{
...
//{
//	INT8U err = 0;
//	OSMutexPend(SendMutex, 0, &err);					//获取互斥锁

//	int16_t size = send(num,pdata,len);  //测试发送数据--返回数据就是
//	if(size != len)
//	{
//		printf("cuowu-size = %d\n",size);
//		OSMutexPost(SendMutex); //释放锁

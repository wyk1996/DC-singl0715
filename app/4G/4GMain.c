...
_NET_CONFIG_INFO  NetConfigInfo[XY_MAX] =
{
    ...
    {XY_YKC,	{121,43,69,62},8767,{0}	,		1},  //YKC云快充
    ...
    {XY_JG,		{114,55,126,89},8000,{0} ,		1},  //精工

    {XY_TT,		{121,36,64,61},8767, {0} ,		1},  //铁塔

    {XY_ZSH,	{123,56,81,41},8767,{0} ,		1},  //ZSH中石化
	
    {XY_YD,	    {47,100,166,66},9000,{0} ,		1},  //绍兴易电
	
    {XY_XDT,	{47,114,93,1},10594,{0} ,		1},  //新电途	 //生产IP 47.114.93.1 10594 
	
    {XY_GZZJ,	{118,178,228,197},8000,{"zj.fengjingit.com"},1},  //贵州中交  //118.178.228.197 8000
	
    {XY_TTC,	{47,103,110,125},8767,{"charge.evchong.com"},1},  //淘淘充  2024年7月3日号对接暂用域名和port
};
...
INT8U UART_4GWrite(INT8U* const FrameBuf, const INT16U FrameLen)
{
    ...
    if((SYSSet.NetYXSelct == XY_ZSH)||(SYSSet.NetYXSelct == XY_XDT)||(SYSSet.NetYXSelct == XY_TTC)||(SYSSet.NetYXSelct == XY_YKC)||(SYSSet.NetYXSelct == XY_TT)||(SYSSet.NetYXSelct == XY_YL3)||(SYSSet.NetYXSelct == XY_YL4))    //连云快充双平台的时候，发送数据很快
        {
...
#define RESET_4G_COUNT	5    //目前连续1次未连接上服务器 ，则重新启动
	...
	if(NetConfigInfo[SYSSet.NetYXSelct].NetNum >= 2)
	{
		if(APP_GetModuleConnectState(0) !=STATE_OK)   //第0个平台检测
		{
			if(++num >= RESET_4G_COUNT)
			{
				num = 0;
	...
	for(count = 0; count < NetConfigInfo[SYSSet.NetYXSelct].NetNum; count++)
	{
		if(APP_GetModuleConnectState(count) != STATE_OK) //位连接服务器
		{
			if(count == 0)
			{
				...
				{
					ModuleSIM7600_ConnectServer(count,(INT8U*)"zj.fengjingit.com",NetConfigInfo[SYSSet.NetYXSelct].port);
				}
				else if(SYSSet.NetYXSelct == XY_TTC)
				{
				  ModuleSIM7600_ConnectServer(count,(INT8U*)"charge.evchong.com",NetConfigInfo[SYSSet.NetYXSelct].port);
				}
				 else
				{
				   ModuleSIM7600_ConnectServer(count,(INT8U*)NetConfigInfo[SYSSet.NetYXSelct].pIp,NetConfigInfo[SYSSet.NetYXSelct].port);
...
//		OSTimeDly(SYS_DELAY_1s);
        ...
        if(APP_GetSIM7600Mode() == MODE_DATA)
        {
            ...
            else
            {
                if(SYSSet.NetState == DISP_NET)
                {
                    ...
                    //网络状态下，如果连续10分钟未连接上，而且未插枪状态-系统自动重启
                    nowSysTime = OSTimeGet();
        ...
        else if((APP_GetSIM7600Mode() == MODE_HTTP) && (SYSSet.Net4GorEternet == NET_4G))
        {
            //远程升级
            if(APP_GetModuleConnectState(0) == STATE_OK)
            {
        ...
        else if((APP_GetSIM7600Mode() == MODE_FTP) && (SYSSet.Net4GorEternet == NET_4G))
        {
            //FTP
            if(APP_GetModuleConnectState(0) == STATE_OK)
            {

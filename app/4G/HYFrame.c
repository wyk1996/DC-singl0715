...
hy_cmd_13_14 SendHYCmd13_14;   //发送和接收SOC是一样的
...
static INT8U   HY_RecvSOC(INT8U *pdata,INT16U len);
...
_4G_RECV_TABLE HYRecvTable[HY_RECV_FRAME_LEN] = {
	...
	{hy_cmd_type_13				,  	HY_RecvSOC	},		//下发SOC
...
* Contributor  : 2021年3月19日
*****************************************************************************/
INT8U   HY_SendSOCAck(_GUN_NUM gun)
{
	INT8U * pdevnum = APP_GetDevNum(); 
	memcpy(SendHYCmd13_14.equipment_id,pdevnum,16);	//充电桩编号
	SendHYCmd13_14.SOCvalue = SYSSet.SOCthreshold;
	return HYFreamSend(hy_cmd_type_14,(INT8U*)&SendHYCmd13_14,sizeof(SendHYCmd13_14));
}
...
static INT8U   HY_RecvSOC(INT8U *pdata,INT16U len)
{
	...
	OS_EVENT* pevent = APP_Get4GMainEvent();
	 static _BSP_MESSAGE SendMsg[GUN_MAX];
	hy_cmd_13_14 * pcmd13_14 = (hy_cmd_13_14 *)pdata;
	if((pdata == NULL) || len != sizeof(hy_cmd_13_14))
	{
		return FALSE;
	}
	
    if(pcmd13_14->SOCvalue > 100)  //SOC的值必须要小于101
	{
	   return FALSE;
	}
	   RECVSOC = pcmd13_14->SOCvalue << 8 | 0x11;
	   Munu52_SOCDispose(0,(INT8U *)&RECVSOC,2);  //直接存储了

		//发送应答SOC
	Msg[gun].MsgID = BSP_4G_MAIN;
	Msg[gun].DataLen = gun;
	Msg[gun].DivNum = APP_SOCValue_ACK;
	OSQPost(psendevent, &Msg[gun]);

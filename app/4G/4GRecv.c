...
OS_EVENT *GPRSRECVTaskEvent;				            // 使用的事件
...
void Task4GRecv(void *pdata)
{
	...
	while(1)
	{
		...
		if (OS_ERR_NONE == err)
		{
			switch (pMsg->MsgID)
			{
				case BSP_MSGID_UART_RXOVER:  //4g串口接收
...
//							}
						...
						else
						{
							...
							else  //连接2个平台
							{
								...
								for(i = 0; i < NetConfigInfo[SYSSet.NetYXSelct].NetNum; i++)
								{
									if(RecvDataControl[i].RecvStatus == RECV_FOUND_DATA)  //接收到数据
									{
				...
				case BSP_MSGID_Ethernet_RXOVER:	 //单独的以太网接收(只在以太网的地方民发送接收)
					if(NetConfigInfo[SYSSet.NetYXSelct].NetNum == 1) //当前1个后台，有线网接受到就应该正常连接了
					{
						if(APP_GetModuleConnectState(0) == STATE_OK) //已经连接上后台了
						{
							if(_4G_RecvFrameDispose(pMsg->pData,pMsg->DataLen))  //数据透传

...
#define	TASK_STK_SIZE_SAM	 0x500       //0x800==原来的	
OS_STK TaskSAMStk[TASK_STK_SIZE_SAM];				//采样任务

#define TASK_STK_SIZE_PERIOD       0x500   //0x800==原来的	
...
OS_STK TaskStkGPRSMainStk[TASK_STK_SIZE_GPRSMIAN];		    // 4g主任务任务栈大小

#define TASK_STK_SIZE_GPRSSEND  0x800
OS_STK TaskStkGPRSSendStk[TASK_STK_SIZE_GPRSSEND];		    // 4G发送任务栈大小

#define TASK_STK_SIZE_GPRSRECV  0x800
OS_STK TaskStkGPRSRecvStk[TASK_STK_SIZE_GPRSRECV];		    // 4G接收任务栈大小
...
#define TASK_STK_SIZE_W5500     0x700        //网口连接
OS_STK TaskW5500Stk[TASK_STK_SIZE_W5500];	//CPU卡任务堆栈


#define TASK_STK_SIZE_TIST  0x800  //模块任务
...
INT8U SYS_APP_VER[3] = {24,7,4};  //屏幕显示软件版本号
...
//				 jump_to_app(LoadCodeAddress); // 跳转到BOOT
	...
	OSTaskCreateExt(TaskPeriod,    //周期性检测任务
	                (void *)0,
	...
	OSTaskCreateExt(TaskSample,     //实时获取AD采样后的值
	                (void *)0,
	...
	OSTaskCreateExt(TaskADJust,   //主要是接收屏幕上面的校准信息(已自动校准)
	                (void *)0,
...
//                    OS_TASK_OPT_STK_CHK + OS_TASK_OPT_STK_CLR);
	...
	OSTaskCreateExt(TaskUsbTask,   //创建任务,USB任务
	                (void *)0, 
	                &TaskUsbStk[0x800 - 1],
	                PRI_USBTASK,
	                PRI_USBTASK,
